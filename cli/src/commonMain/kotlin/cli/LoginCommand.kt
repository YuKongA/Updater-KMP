...
class LoginCommand : CliktCommand(name = "login") {
    ...
    override fun run() {
        ...
        val exit = runBlocking {
            handle(t, AppContainer.loginService.login(account, password, global, savePassword))
        }
    ...
    private suspend fun handle(t: Terminal, result: LoginResult): Int = when (result) {
        is LoginResult.Success -> {
            AppContainer.session.save(result.loginData)
            AppContainer.loginFlow.clearTransient() // drop identity_session / 2FA context, matching the GUI contract
    ...
    private suspend fun runTwoFactor(t: Terminal, options: List<Int>): Int {
        ...
        t.println("Two-factor verification required. Methods: " +
        ...
        if (!AppContainer.loginService.sendTicket(flag)) {
        ...
        repeat(3) {
            ...
            val result = AppContainer.loginService.login(
